...
export function educationDayBlocked(account, date, history = []) {
  ...
  return date < account.educationStrategy.startDate || history.some(h => h.mediaId && Number.isFinite(Date.parse(h.publishedAt)) && new Date(h.publishedAt).toLocaleDateString('en-CA', {timeZone:'America/Sao_Paulo'}) === date);
}
...
export function nextEducationPack(history = [], duplicate = () => false, kind = 'tutorial') {
  const candidates = kind === 'offer' ? [0,1,2].map(offerPack) : educationLessons();
  return candidates.find(p => !history.some(h => h.mediaId && (h.education?.lessonId === p.education.lessonId || (p.editorialSeries && h.editorialSeries?.id === p.editorialSeries.id && h.editorialSeries?.episode === p.editorialSeries.episode))) && !duplicate(p)) || null;
}
