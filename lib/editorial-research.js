...
function compactFactualStatement(value = '', maxLength = 105) {
...
export function buildResearchPack(item, now = new Date(), options = {}) {
  ...
  if (customRadar) {
    ...
    const sourceSignal = compactFactualStatement(news.fact);
    const readableSourceSignal = news.fact;
    ...
    return {
      ...
      slides: [
        ...
        { eyebrow: 'ANTES DE APLICAR', title: 'Contexto primeiro. Ferramenta depois.', body: business.trust },
...
export async function researchFreshEditorialPacks(options = {}) {
  ...
  return { packs: factualItems.slice(0, options.limit || 13).map((item) => buildResearchPack(item, now, options)), items: factualItems, failures, researchedAt: now.toISOString() };
}
