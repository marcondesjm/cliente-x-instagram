...
export function buildResearchPack(item, now = new Date(), options = {}) {
  ...
  if (customRadar) {
    ...
    const coverHooks = [
      ...
      'Quanto tempo sua equipe perde procurando a resposta certa?',
      'O cliente espera enquanto sua equipe procura informação?',
      'Uma resposta atrasada também pode custar uma oportunidade.',
      'Sua operação ajuda a equipe ou cria mais trabalho?',
      'O próximo passo está claro para toda a equipe?',
      'Trabalho repetido ocupa o tempo que deveria ir para o cliente.',
      'Informação espalhada deixa a operação mais lenta.',
      'Atender melhor começa antes da conversa com o cliente.',
      'Sua equipe trabalha muito. O processo acompanha esse esforço?',
      'A rotina cresceu. E o caminho da informação?',
      'Nem todo atraso começa no atendimento.',
      'O cliente percebe quando a operação não está conectada.',
      'Uma pequena demora pode virar uma oportunidade perdida.',
      'Mais ferramentas nem sempre significam respostas mais rápidas.',
      'Sua equipe sabe exatamente qual é o próximo passo?'
    ];
  ...
  const itemsByRecency = results.flatMap((result) => result.status === 'fulfilled' ? result.value : [])
    .filter((item) => {
      ...
      const matchesNiche = includeTerms.length
        ? includeTerms.some((term) => includesEditorialTermOrAlias(titleSearchable, term))
