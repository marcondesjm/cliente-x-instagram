...
export function isPredominantlyEnglish(value = '') {
  ...
  const english = text.match(/\b(the|and|with|for|from|into|this|that|how|what|where|why|learn|build|building|workflows|provides|needed|understand|more|your|their|they|are|can|when|which|getting|better|handling|tasks|harder|debug|captures|full|execution|including|model|calls|tool|tools|invocations|visibility|reliable|failures)\b/g)?.length || 0;
  ...
  return english >= 4 && english > portuguese;
}
...
function localizedNews(item = {}, theme = {}) {
  ...
  return {
    ...
    fact: (!isPredominantlyEnglish(realSummary) && realSummary) || match?.fact || '',
...
function businessNarrative(theme = {}, options = {}) {
  ...
  return {
    ...
    cta: `Se isso acontece na sua empresa, vamos olhar juntos para a rotina que mais pesa. Eu ajudo empresas com ${humanOffer}. Me chame no Direct.`
  };
}
...
export function buildResearchPack(item, now = new Date(), options = {}) {
  ...
  if (customRadar) {
    ...
    const originalTitleLabel = isPredominantlyEnglish(item.title) ? 'Título original (em inglês)' : 'Título original';
    const sourceReference = `Fonte consultada: ${item.source}\n${originalTitleLabel}: ${item.title}\n${item.url}`;
    ...
    return {
      ...
      slides: [
        ...
        { eyebrow: 'O QUE A MATÉRIA DIZ', title: sourceSignal, body: `Fonte: ${item.source}. Título original e link completo na legenda.` },
        sourceContinuation
        ...
        { eyebrow: 'FONTE E PRÓXIMO PASSO', title: `Leia a matéria original da ${item.source}.`, body: `${business.cta} O link completo está na legenda.` }
...
export async function researchFreshEditorialPacks(options = {}) {
  ...
  const enrichedResults = await Promise.allSettled(candidates.map(async (item) => {
    ...
    try {
      ...
      const articleFacts = extractArticleFacts(await response.text(), item);
      return articleFacts.length >= 2 ? { ...item, articleFacts } : item;
    } finally {
      clearTimeout(timer);
    }
  }));
