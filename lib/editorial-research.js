...
export const EDITORIAL_SOURCES = [
  ...
  { name: 'Hardware.com.br', url: 'https://www.hardware.com.br/feed/' },
  ...
  { name: 'TI Inside', url: 'https://tiinside.com.br/feed/' },
  { name: 'Startups', url: 'https://startups.com.br/feed/' },
  { name: 'Mobile Time', url: 'https://www.mobiletime.com.br/feed/' },
  { name: 'Google Brasil', url: 'https://blog.google/intl/pt-br/rss/' },
  { name: 'Microsoft Brasil', url: 'https://news.microsoft.com/pt-br/feed/' },
  { name: 'Nossa IA', url: 'https://nossa.ia.br/rss.xml' },
...
const HTML_ENTITY_CHARACTERS = new Map(Object.entries({
...
export function decodeEditorialEntities(value = '') {
  let decoded = String(value);
  for (let pass = 0; pass < 2; pass += 1) {
    decoded = decoded.replace(/&(?:#(\d+)|#x([0-9a-f]+)|([a-z][a-z0-9]+));/gi, (entity, decimal, hexadecimal, named) => {
      ...
      return HTML_ENTITY_CHARACTERS.get(named) || decodeHTMLStrict(entity);
    });
  }
  return decoded;
}

export function filterValidEditorialPacks(packs, validate, onRejected = () => {}) {
  return packs.filter((pack) => {
    try {
      validate(pack);
      return true;
    } catch (error) {
      onRejected(pack, error);
      return false;
    }
  });
}
...
function decodeXml(value = '') {
  return decodeEditorialEntities(String(value)
...
function acceptableEditorialImageUrl(value = '', baseUrl = '') {
  ...
  try {
    ...
    if (/(^|\.)(?:youtube\.com|youtu\.be|vimeo\.com)$/.test(host) || /\/(?:embed|video|player)\//i.test(parsed.pathname)) return '';
  } catch {
    return '';
  }
  return url;
...
export function factualSummary(item = {}) {
  ...
  if (isIncompleteEditorialFact(summary) || isEditorialMediaCaption(summary)) return '';
  if (summary.length <= 520) return summary;
  const completeSentences = summary.match(/[^.!?]+[.!?]+(?:["”'])?/g) || [];
  const compact = completeSentences.reduce((result, sentence) => {
    const next = `${result} ${sentence.trim()}`.trim();
    return next.length <= 520 ? next : result;
  }, '');
  return compact.length >= 45 ? compact : '';
}

export function isIncompleteEditorialFact(value = '') {
  return /(?:\.{3}|…)(?:["”']?\s*)$/.test(String(value || '').trim());
}

export function isEditorialMediaCaption(value = '') {
  const text = String(value || '').replace(/\s+/g, ' ').trim();
  return /(?:—|-)\s*(?:foto|imagem)\s*:/i.test(text)
    || /^(?:foto|imagem|ilustra[cç][aã]o|reprodu[cç][aã]o|o logotipo|a imagem|na imagem)\b.*\b(?:foto|arquivo|reuters|associated press|\bap\/|divulga[cç][aã]o)\b/i.test(text);
}

export function isUsableEditorialFact(value = '') {
  const text = String(value || '').replace(/\s+/g, ' ').trim();
  return text.length >= 45 && !isIncompleteEditorialFact(text) && !isEditorialMediaCaption(text);
}

function stripTrailingEditorialMediaCredit(value = '') {
  return String(value || '')
    .replace(/\s*\((?:foto|imagem|ilustra[cç][aã]o|reprodu[cç][aã]o)\b[^)]*\)\s*$/iu, '')
    .trim();
}
...
export function extractArticleFacts(html = '', item = {}) {
  ...
  const paragraphs = [...String(html).matchAll(/<p(?:\s[^>]*)?>([\s\S]*?)<\/p>/gi)]
    ...
    // Alguns portais anexam o crédito da fotografia ao fim do primeiro
    // parágrafo. O crédito não faz parte do fato e não pode chegar ao card.
    .map(stripTrailingEditorialMediaCredit)
    .filter((text) => text.length >= 70 && text.length <= 520 && !rejected.test(text) && isUsableEditorialFact(text));
  const seen = new Set();
...
function compactFactualStatement(value = '', maxLength = 156) {
  const clean = String(value || '').replace(/\s+/g, ' ').trim();
  const sentence = clean.split(/(?<=[.!?])\s+/).filter(Boolean)[0] || clean;
  if (sentence.length <= maxLength) return /[.!?]$/.test(sentence) ? sentence : `${sentence}.`;

  // Preserve uma ideia completa no cartão. Em vez de cortar uma palavra e
  // terminar com reticências, procura o fim de uma oração factual e fecha a
  // frase. O texto integral e a fonte continuam disponíveis na legenda.
  const window = sentence.slice(0, maxLength + 1);
  const boundaries = [...window.matchAll(/[,;:]\s+|\s+[—–-]\s+|\s+(?:mas|porém|enquanto|embora|e sobre|e afirmou que|e explicou que)\s+/giu)]
    .map((match) => match.index)
    .filter((index) => index >= 58 && index <= maxLength);
  const boundary = boundaries.at(-1);
  const fallback = window.slice(0, maxLength).replace(/\s+\S*$/, '').trim();
  const compact = String(boundary ? sentence.slice(0, boundary) : fallback)
    .replace(/[,:;\s]+$/g, '')
    .replace(/\b(?:e|de|da|do|das|dos|com|para|sobre|que)$/iu, '')
    .trim();
  return `${compact}.`;
}
...
function includesEditorialTerm(text = '', term = '') {
  const normalizeSearch = (value, preserveAccents = false) => {
    const lowered = String(value || '').toLowerCase();
    return preserveAccents
      ? lowered
      : lowered.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  };
  // O termo inglês "AI" não pode aceitar "aí" como inteligência artificial.
  // Preservar acentos em tokens de duas letras evita esse falso positivo.
  const preserveAccents = String(term || '').trim().length <= 2;
  const normalizedText = normalizeSearch(text, preserveAccents);
  const cleanTerm = normalizeSearch(term, preserveAccents).trim();
...
export function buildResearchPack(item, now = new Date(), options = {}) {
  ...
  if (customRadar) {
    ...
    const articleFacts = Array.isArray(item.articleFacts)
      ? item.articleFacts.filter((fact) => isUsableEditorialFact(fact) && !isPredominantlyEnglish(fact))
      : [];
    const localizedFacts = (news.facts.length >= 2 ? news.facts : [news.fact]).filter((fact) => isUsableEditorialFact(fact));
    const sourceFacts = articleFacts.length >= 2 ? articleFacts : localizedFacts;
    ...
    const editorialVariants = [
      { cover: 'NOTÍCIA', fact: 'O PONTO CENTRAL', continuation: 'A MATÉRIA CONTINUA', analysis: 'LEITURA PARA EMPRESAS', analysisTitle: 'O que isso muda na rotina?', intro: 'Vale olhar com calma para esta pauta.', bridge: 'Trazendo o assunto para a operação:' },
      { cover: 'EM PAUTA', fact: 'O QUE ACONTECEU', continuation: 'OUTRO PONTO IMPORTANTE', analysis: 'E NA PRÁTICA?', analysisTitle: 'Onde essa mudança encontra a empresa?', intro: 'Uma notícia para entender sem pressa e sem exagero.', bridge: 'Agora, separando fato de análise:' },
      { cover: 'VALE ENTENDER', fact: 'EM POUCAS PALAVRAS', continuation: 'TEM MAIS CONTEXTO', analysis: 'OLHANDO PARA A ROTINA', analysisTitle: 'A pergunta que fica para as empresas', intro: 'O título chama atenção, mas o contexto é o que importa.', bridge: 'Nossa leitura prática, além do que diz a fonte:' },
      { cover: 'CONTEXTO', fact: 'PRIMEIRO, O FATO', continuation: 'A MATÉRIA AVANÇA', analysis: 'DO FATO À ROTINA', analysisTitle: 'Como isso pode aparecer no dia a dia?', intro: 'Vamos direto ao que a matéria realmente apresenta.', bridge: 'O possível reflexo para empresas é este:' },
      { cover: 'LEITURA DO DIA', fact: 'O DADO QUE IMPORTA', continuation: 'VALE NOTAR TAMBÉM', analysis: 'UMA LEITURA POSSÍVEL', analysisTitle: 'Que conversa essa pauta abre?', intro: 'Esta pauta merece contexto, não apenas uma manchete.', bridge: 'Fora da notícia, uma aplicação possível:' },
      { cover: 'O QUE CHAMOU ATENÇÃO', fact: 'PARA ENTENDER', continuation: 'SEGUNDO PONTO', analysis: 'TRAZENDO PARA A EMPRESA', analysisTitle: 'Onde vale prestar atenção agora?', intro: 'Aqui está o essencial da pauta, com a fonte aberta para conferência.', bridge: 'Na rotina empresarial, nossa análise é:' }
    ];
    const variantIndex = Number.parseInt(createHash('sha256').update(item.url).digest('hex').slice(12, 20), 16) % editorialVariants.length;
    const editorial = editorialVariants[variantIndex];
    const factSlide = { eyebrow: editorial.fact, title: sourceSignal, body: `Fonte: ${item.source}. Matéria publicada em ${dateLabel}.` };
    const continuationSlide = sourceContinuation
      ? { eyebrow: editorial.continuation, title: compactFactualStatement(sourceContinuation), body: `Informação da matéria original de ${item.source}.` }
      : { eyebrow: editorial.analysis, title: `Onde ${theme.label} pode entrar na rotina?`, body: `A conexão possível é ${theme.application}. Esta é uma análise editorial, não uma afirmação da matéria.` };
    const analysisSlide = { eyebrow: editorial.analysis, title: editorial.analysisTitle, body: `A conexão possível é ${theme.application}. Esta é uma análise editorial separada da notícia.` };
    const middleSlides = variantIndex % 2 === 0
      ? [factSlide, continuationSlide, analysisSlide]
      : [factSlide, analysisSlide, continuationSlide];
    return {
      ...
      research: {
        ...
        sourceFacts,
        editorialVariant: variantIndex
      },
      slides: [
        { eyebrow: `${editorial.cover} · ${item.source}`, title: coverHook, body: `Publicada em ${dateLabel}. ${editorial.intro}` },
        ...middleSlides,
  ...
  return {
    ...
    slides: [
      ...
      { eyebrow: 'COMEÇO CERTO', title: 'Comece por um fluxo pequeno e mensurável.', body: 'Escolha uma rotina repetitiva, defina entrada, resultado esperado e quem confere as exceções.' },
...
export async function researchFreshEditorialPacks(options = {}) {
  ...
  // Keep a broad pool so 13 daily slots do not exhaust a single dominant
  // portal before the semantic and URL history guards have alternatives.
  const candidates = factualItems.slice(0, options.limit || 80);
