...
export function buildResearchPack(item, now = new Date(), options = {}) {
  ...
  if (customRadar) {
    ...
    return {
      ...
      slides: [
        { eyebrow: 'ISSO ACONTECE AÍ?', title: business.hook, body: `${business.pain} ${business.connection}` },
        { eyebrow: 'O QUE A NOTÍCIA MOSTRA', title: business.impact, body: `${item.source} mostra que ${readableSourceSignal} Publicado em ${dateLabel}.` },
        { eyebrow: 'O QUE MUDA NA ROTINA', title: business.solution, body: `${business.solutionDetail} ${business.clarity}` },
        { eyebrow: 'UM CAMINHO SEGURO', title: 'Mudança boa não assusta a equipe.', body: business.trust },
        { eyebrow: 'VAMOS CONVERSAR?', title: 'Você não precisa resolver tudo sozinho.', body: business.cta }
      ],
