...
export function isPredominantlyEnglish(value = '') {
  ...
  const english = text.match(/\b(the|and|with|for|from|into|this|that|how|what|where|why|learn|build|building|workflow|workflows|automation|comparing|versus|provides|needed|understand|more|your|their|they|are|can|when|which|get|getting|better|handling|tasks|harder|debug|captures|full|execution|including|model|calls|tool|tools|invocations|visibility|reliable|reliability|security|scalability|observability|operations|long|term|failures)\b/g)?.length || 0;
