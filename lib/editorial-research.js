...
export function buildResearchPack(item, now = new Date(), options = {}) {
  ...
  if (customRadar) {
    ...
    const factSlide = { eyebrow: editorial.fact, title: sourceSignal, body: `Esse é um fato apresentado pela ${item.source}; o link completo está na legenda.` };
    const continuationSlide = sourceContinuation
      ? { eyebrow: editorial.continuation, title: compactFactualStatement(sourceContinuation), body: `Este ponto também vem da matéria publicada pela ${item.source}.` }
