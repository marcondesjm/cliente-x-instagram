...
export function buildResearchPack(item, now = new Date(), options = {}) {
  ...
  if (customRadar) {
    ...
    const articleFacts = Array.isArray(item.articleFacts) ? item.articleFacts.filter(Boolean) : [];
    const sourceFacts = articleFacts.length >= 2 ? articleFacts : [news.fact].filter(Boolean);
    ...
    const sourceReference = `Fonte: ${item.source} · link da matéria no primeiro comentário.`;
    ...
    return {
      ...
      slides: [
        ...
        { eyebrow: 'O QUE A MATÉRIA DIZ', title: sourceSignal, body: `Fonte: ${item.source}. Título original e link completo no primeiro comentário.` },
        sourceContinuation
        ...
        { eyebrow: 'FONTE E PRÓXIMO PASSO', title: `Leia a matéria original da ${item.source}.`, body: `${business.cta} O link completo está no primeiro comentário.` }
