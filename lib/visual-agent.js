export const CLOUD_VISUAL_AGENT_VERSION = 'nerion-visual-agent-v1.0.1';
...
export function buildVisualAgentPlan(pack = {}, visualSources = []) {
  ...
  return {
    ...
    status: sourceUrl && !cover ? 'blocked' : 'approved',
...
export function assertVisualAgentPlan(pack = {}, plan = {}) {
  if (pack?.research?.sourceUrl && (plan.status !== 'approved' || !plan.slideImagePaths?.[0] || plan.approvedVisuals !== 1)) {
    throw new Error('Agente Visual bloqueou a publicação: a capa precisa de uma foto da matéria, disponível e não repetida.');
  }
