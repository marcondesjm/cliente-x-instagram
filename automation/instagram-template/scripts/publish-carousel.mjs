...
function recordPublicationHistory(configDir, accountKey, pack, result) {
  ...
  if (!entries.some((entry) => entry.feedFingerprint === fingerprint || entry.storyFingerprint === fingerprint)) {
    entries.push({
      ...
      slotIndex: Number.isInteger(Number(result.slotIndex)) ? Number(result.slotIndex) : null,
...
async function main() {
  ...
  const renderContext = {
    dateString: today,
    slotIndex,
    slotDate: process.env.INSTAGRAM_TEMPLATE_SLOT_DATE || today,
    scheduledAt: process.env.INSTAGRAM_TEMPLATE_SCHEDULED_AT || scheduledPost?.scheduledFor || null,
