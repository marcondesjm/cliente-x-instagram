(() => {
  ...
  window.loadGrowthPanel = async () => {
    ...
    try {
      ...
      el('growthStrategy').textContent = result.strategy?.enabled ? result.strategy.mode === 'integrated-slot' ? `Agenda de ${result.scheduleCount} postagens diárias: aula em carrossel às 16h BRT, incluída nos horários, com Story complementar. Aulas a partir de ${result.strategy.startDate}; aprendizado educativo mantido. Os demais horários continuam com o Radar.` : `Estratégia educativa ativa: um carrossel às 16h BRT com Story complementar. Início: ${result.strategy.startDate}. Revisão: ${result.strategy.reviewDate}. Ciclo de 10 posts: 7 tutoriais, 2 notícias aplicáveis e 1 oferta. Sem notícia adequada, entra tutorial.` : 'Estratégia educativa ainda não ativada nesta conta.';
